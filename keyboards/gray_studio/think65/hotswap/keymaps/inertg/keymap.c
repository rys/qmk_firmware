...
enum inertg_keycodes {
    INERT_G = THINK65_SAFE_RANGE
};
    ...
    switch (keycode) {
        case INERT_G:
            if (record->event.pressed) {
                // light LED colours
                rgblight_setrgb_at(20, 0, 51, 0);
                rgblight_setrgb_at(14, 2, 63, 1);
                rgblight_setrgb_at(0, 29, 45, 2);
                rgblight_setrgb_at(0, 40, 38, 3);
                // badge LED colours
                rgblight_setrgb_at(105, 0, 210, 4);
                rgblight_setrgb_at(60, 10, 250, 5);
                rgblight_setrgb_at(0, 120, 190, 6);
                rgblight_setrgb_at(0, 165, 155, 7);
            }
            break;
        default:
            break;
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [1] = LAYOUT_65_ansi_blocker(
        ...
        _______, _______, _______,                      INERT_G,                         _______, _______,          TOG_ESC, TOG_BDG, TOG_UGL
    ),
